...
class VideoManager:
    ...
    def __init__(self, cfg):
        ...
        # 비디오 파일 지정하는 부분, 이부분 slowfast_detection.py로 이동(video_path_input) video_path_input 에서
        # 리스트에서 클릭한 비디오 받아오기 위함
        self.source = (
        ...
        self.seq_length = cfg.DATA.NUM_FRAMES * cfg.DATA.SAMPLING_RATE # 8 * 8
    ...
    def __next__(self): # iter 하기 때문에 VideoManger 가 init 된 후 부터 __next__를 불러오게됨
        """
        ...
        was_read = True
        while was_read and len(frames) < self.seq_length: # self.seq_length ==> 64 (yaml 파일 안 DATA.NUM_FRAMES * DATA.SAMPLING_RATE)
