...
def draw_camera_object_groupbox(): # 객체 통계 그룹박스 그리기 함수
    ...
    total_object_count_num = QLabel("0", objectName='total_object_count') # 추후 init_ui에선 objectName을 통해 해당 라벨 접근 가능
    '''
    ...
    person_count = QLabel("Person Count : ")
    person_count_num = QLabel("0", objectName='person_count')
    person_hbox.addWidget(person_count)
    person_hbox.addWidget(person_count_num)

    # detection car count
    car_count = QLabel("Car Count : ")
    car_count_num = QLabel("0", objectName='car_count')
    car_hbox.addWidget(car_count)
    car_hbox.addWidget(car_count_num)

    # detection boat count
    boat_count = QLabel("Boat Count : ")
    boat_count_num = QLabel("0", objectName='boat_count')
...
def draw_camera_action_groupbox(): # 액션 통계 그룹박스 그리기 함수
    ...
    total_action_count_num = QLabel("0", objectName='total_action_count')
    total_action_hbox.addWidget(total_action_count)
    total_action_hbox.addWidget(total_action_count_num)

    # detection person count
    sos_count = QLabel("SOS Count : ")
    sos_count_num = QLabel("0", objectName='sos_count')
    sos_hbox.addWidget(sos_count)
    sos_hbox.addWidget(sos_count_num)

    # detection car count
    fall_down_count = QLabel("Fall Down Count : ")
    fall_down_count_num = QLabel("0", objectName='fall_down_count')
