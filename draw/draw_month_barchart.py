...
def draw_month_barchart():  # barchar 그리기 함수
    pg.setConfigOptions(background='black', foreground='w')  # pyqtgraph 옵션 설정(전경,배경 색 지정)

    month_axis = np.arange(1, 13)  # x축 데이터 생성 (1~12)
    data = np.random.randint(100, size=12)  # y축 랜덤 데이터 생성 <- 월별 탐지 객체수 데이터 삽입 장소

    pw = pg.PlotWidget(title="Detect Count by Month")
    ...
    barchar = pg.BarGraphItem(x=month_axis, height=data, width=0.8, brush=(31, 56, 120), pen='w')  # barchart 생성 / x축 = 1~12 / y축 = 랜덤 데이터 12개
    pw.addItem(barchar)
